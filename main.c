...
/*   Updated: 2015/05/27 23:53:47 by lboudaa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test_bzero(int *flag)
{
	...
	while (nb < 3)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\" and %d :\033[00m ", (nb + 1), tab[nb], tab2[nb]);
		if (launch_bzero(tab[nb], tab2[nb]) == 1)
			dprintf(1, "\033[32mOK\033[00m\n");
		else
		{
			(*flag)++;
			dprintf(1, "\033[31mERROR\033[00m\n");
		}
		nb++;
	}
}
...
void	test_strcat(int *flag)
{
	...
	while (nb < 5)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\" and \"%s\" :\033[00m ", (nb + 1), tab[nb], tab2[nb]);
...
void	test_isalpha(int *flag)
{
	...
	{
		if (tab[nb] < 0)
			dprintf(1, "\033[94mTest number %d with < 0   :\033[00m ", (nb + 1));
		else if (tab[nb] > 127)
			dprintf(1, "\033[94mTest number %d with >127  :\033[00m ", (nb + 1));			
		else if (tab[nb] == 0)
			dprintf(1, "\033[94mTest number %d with NULL  :\033[00m ", (nb + 1));
		else
			dprintf(1, "\033[94mTest number %d with '%c'   :\033[00m ", (nb + 1), tab[nb]);
...
void	test_puts()
{
	...
	while (nb < 4)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\" :\033[00m\n", (nb + 1), tab[nb]);
		launch_puts(tab[nb]);
		nb++;
	}
	dprintf(1, "\033[94mTest number %d with \"Bonjour\" and we close STDOUT :\033[00m\n", (nb + 1));
	close(1);
	i = ft_puts("Bonjour");
	j = puts("Bonjour");
	dup2(0, 1);
	dprintf(1, "\033[92mft_puts return : %d\033[00m\n\033[91mputs return : %d\033[00m\n", i, j);
}
...
void	test_strlen(int *flag)
{
	...
	while (nb < 4)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\" : \033[00m", (nb + 1), tab[nb]);
...
void	test_memset(int *flag)
{
	...
	while (nb < 5)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\", '%c' and %d :\033[00m ", (nb + 1), tab[nb], tab2[nb], tab3[nb]);
...
void	test_memcpy(int *flag)
{
	...
	while (nb < 4)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\", \"%s\" and %d :\033[00m ", (nb + 1), tab[nb], tab2[nb], tab3[nb]);
...
void	test_cat()
{
	...
	nb = 0;
	dprintf(1, "\033[94mTest number %d with fd = 0 :\033[00m \nEnter a text and \"Control + D\" to do the other tests : \n", (nb + 1));
	launch_cat(0);
	nb++;
	...
	dprintf(1, "\033[94mTest number %d with fd = %d :\033[00m \n", (nb + 1), fd);
	launch_cat(fd);
	close(fd);
	dprintf(1, "\n");
	nb++;
...
/* TEST FT_STRCMP */
int		launch_strcmp(char *s1, char *s2)
{
	int		a;
	int		b;
	a = ft_strcmp(s1, s2);
...
{
	int		nb;
	char	*tab[] = {"Hello World !", "Coucou", ft_strdup(""), ft_strdup("12*456789"), ft_strdup("123456789")};
	char	*tab2[] = {"Hello", "Coucou", ft_strdup(""), ft_strdup("123456780"), ft_strdup("120456789")};
	...
	{
		dprintf(1, "\033[94mTest number %d with \"%s\" and \"%s\" :\033[00m ", (nb + 1), tab[nb], tab2[nb]);
		if (launch_strcmp(tab[nb], tab2[nb]) == 1)
			dprintf(1, "\033[32mOK\033[00m\n");
		else
		{
			(*flag)++;
...
/* TEST FT_STREQU */
int		launch_strequ(char *s1, char *s2, int n)
{
	if (ft_strequ(s1, s2) == n)
		return (1);
	else
		return (0);
}

void	test_strequ(int *flag)
{
	int		nb;
	char	*tab[] = {"Hello World !", "Coucou", "", "123456789", "123456789"};
	char	*tab2[] = {"Hello", "Coucou", "", "123456780", "120456789"};
	int		tab3[] = {0, 1, 1, 0, 0};

	dprintf(1, "\n\033[33m------- Test FT_STREQU -------\033[00m\n");
	nb = 0;
	while (nb < 5)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\", \"%s\" and %d :\033[00m \n", (nb + 1), tab[nb], tab2[nb], tab3[nb]);
		if (launch_strequ(tab[nb], tab2[nb], tab3[nb]) == 1)
			dprintf(1, "\033[32mOK\033[00m\n");
		else
		{
			(*flag)++;
...
/* TEST FT_STRCHR */
int		launch_strchr(char *str, int c)
{
...
void	test_strchr(int *flag)
{
	...
	while (nb < 4)
	{
		dprintf(1, "\033[94mTest number %d with \"%s\" and '%c' :\033[00m\n", (nb + 1), tab[nb], tab2[nb]);
		if (launch_strchr(tab[nb], tab2[nb]) == 1)
			dprintf(1, "\033[32mOK\033[00m\n");
		else
		{
			(*flag)++;
...
int		main(int ac, char **av)
{
	...
	if (flag == 0)
		dprintf(1, "\033[32mAll tests are good :D\033[00m\n");
	else
		dprintf(1, "\033[31m%d tests failed :(\033[00m\n", flag);
	dprintf(1, "\n");
	return (0);
}
